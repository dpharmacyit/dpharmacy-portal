// firebase-config.js
// ⚠️ ไฟล์นี้ไม่มี API Key
// Key จะถูกโหลดจาก config.js แยกต่างหาก

// Initialize Firebase (config โหลดจาก config.js)
...
function calculateExpiryDate(membershipType, customValue = null) {
  ...
  if (typeof membershipType === 'string' && membershipType.startsWith('custom_')) {
    ...
    if (membershipType === 'custom_days')   { now.setDate(now.getDate() + num); return now; }
    if (membershipType === 'custom_weeks')  { now.setDate(now.getDate() + (num * 7)); return now; }
    if (membershipType === 'custom_months') { now.setMonth(now.getMonth() + num); return now; }
    if (membershipType === 'custom_years')  { now.setFullYear(now.getFullYear() + num); return now; }
  }
...
function formatMembershipType(type, customValue = null) {
  const map = { day:'1 วัน', week:'1 สัปดาห์', month:'1 เดือน', year:'1 ปี', permanent:'ถาวร' };
...
async function logActivity(action, details = {}) {
  try {
    ...
    await db.collection('audit_logs').add({
      uid: user.uid, email: user.email, action, details,
...
function requireAuth(callback) {
  auth.onAuthStateChanged(async (user) => {
    ...
    if (!userData) { alert('ไม่พบข้อมูลผู้ใช้'); auth.signOut(); return; }
    if (!userData.isActive) { alert('บัญชีถูกระงับ'); auth.signOut(); return; }
    const expiryCheck = checkExpiry(userData.expiresAt);
    if (!expiryCheck.valid) { alert('บัญชีหมดอายุ'); auth.signOut(); return; }
    callback(userData);
  });
}
